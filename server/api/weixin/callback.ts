...
export const _outKFCallback = async (req: Request): Promise<Response> => {
  ...
  if (req.method === "POST") {
    try {
      ...
      logger.log('原始请求体:', body.substring(0, 200) + '...')
      ...
      logger.log('签名参数:', { msgSignature, timestamp, nonce })
      
      const encryptMatch = body.match(/<Encrypt><!\[CDATA\[(.*?)\]\]><\/Encrypt>/)
      if (encryptMatch) {
        logger.log('检测到加密消息，正在解密...')
        const encrypt = encryptMatch[1]
        const { message, id } = decrypt(WECOM_ENCODING_AES_KEY, encrypt)
        logger.log('解密后的完整消息:', message)
        
        const callbackEvent = await parseXml(message)
        logger.log('解析结果:', JSON.stringify(callbackEvent, null, 2))
        logger.log('事件类型:', callbackEvent.Event)
        logger.log('客服账号:', callbackEvent.OpenKfId)
        logger.log('Token:', callbackEvent.Token ? callbackEvent.Token.substring(0, 20) + '...' : 'null')
        ...
        if (callbackEvent.Event === 'kf_msg_or_event') {
          ...
          logger.log(`还有更多: ${syncResponse.has_more === 1 ? '是' : '否'}`)
          ...
          while (syncResponse.has_more === 1) {
            ...
            logger.log(`拉取到 ${nextMsgList.length} 条消息`)
        ...
        logger.log('未检测到加密，尝试直接解析...')
        const callbackEvent = await parseXml(body)
        logger.log('事件类型:', callbackEvent.Event)
        logger.log('客服账号:', callbackEvent.OpenKfId)
        logger.log('Token:', callbackEvent.Token.substring(0, 20) + '...')
      }
