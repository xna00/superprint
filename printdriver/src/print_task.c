    ...
    for (int i = 0; i < array_len; i++) {
        ...
        if (json_object_object_get_ex(task, "id", &task_id_obj) &&
            ...
            for (int j = 0; j < file_count; j++) {
                ...
                if (json_object_object_get_ex(file, "id", &file_id_obj) &&
                    json_object_object_get_ex(file, "fileId", &file_id_obj)) {
                    
                    int file_id_int = json_object_get_int(file_id_obj);
                    snprintf((*files)[*count].print_task_id, sizeof((*files)[*count].print_task_id), "%s", print_task_id);
                    snprintf((*files)[*count].file_id, sizeof((*files)[*count].file_id), "%d", file_id_int);
                    ...
                    if (json_object_object_get_ex(file, "filename", &filename_obj)) {
                        ...
                        if (fname) {
                            ...
                            snprintf((*files)[*count].filename, sizeof((*files)[*count].filename), "%d.ps", file_id_int);
                        }
                    } else {
                        snprintf((*files)[*count].filename, sizeof((*files)[*count].filename), "%d.ps", file_id_int);
                    }
